...
c.ServerApp.allow_origin_pat = r"(^https://8080-dot-[0-9]+-dot-devshell\.appspot\.com$)|(^https://colab\.(?:sandbox|research)\.google\.com$)|(^(https?://)ssh\.cloud\.google\.com/devshell$)"
...
def _get_session(prefix="http://", max_retries=MAX_RETRIES):
  ...
  Args: prefix(str): Prefix for URL max_retries(int): Maximum number of retries
  each connection should attempt.
...
def get_attribute_value(attribute):
  ...
  Args: attribute(str): Attribute key to look in Compute Metadata.
...
def read_from_file(path):
  ...
  Args: path(str): Location of file with metadata information.
...
def metadata_env_pre_save(model, **kwargs):  # pylint: disable=unused-argument
  ...
  Args: model(dict): Notebooks information
...
import nb_micromamba_kernels
c.MixingKernelSpecManager.local_kernel_spec_manager_class = nb_micromamba_kernels.MicroMambaKernelSpecManager
c.ServerApp.kernel_spec_manager_class = nb_micromamba_kernels.MicroMambaKernelSpecManager
c.MicroMambaKernelSpecManager.name_format = '{display_name}'
